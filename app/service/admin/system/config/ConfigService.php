...
{
    /**
     * 配置读取时需要移除的数据权限 Scope
     *
     * 系统配置属于全局数据，非 SaaS 多租户模式下不做数据权限隔离。
     * 所有读取入口统一移除该 Scope，保证任何调用方（无论是否登录、
     * 拥有何种数据权限）拿到的配置完全一致。
     */
    private const WITHOUT_SCOPES = [AccessPermissionScope::class];
    ...
    public function config(string $code, mixed $default = [], array $options = []): mixed
    {
        ...
        // 查询配置（移除数据权限 Scope）
        $configModel = $this->dao->get($map, ['*'], [], '', self::WITHOUT_SCOPES);
    ...
    public function getByGroup(string $groupCode, array $defaults = [], array $options = []): array
    {
        ...
        if ($withMetadata) {
            // 获取完整配置信息（移除数据权限 Scope）
            $configs       = $this->dao->getList($map, '*', 0, 0, '', [], false, self::WITHOUT_SCOPES);
            $processedData = [];
            ...
            // 只获取配置内容（移除数据权限 Scope，与其它读取入口保持一致）
            $configs = $this->dao->getList($map, '*', 0, 0, '', [], false, self::WITHOUT_SCOPES);
            $data    = [];
            foreach ($configs as $config) {
                $data[$config['code']] = $config['content'];
            }
    ...
    public function update(string $code, mixed $content, array $options = []): void
    {
        try {
            ...
            $map         = ['code' => $code];
            // 移除数据权限 Scope，否则查不到已存在记录会误建重复配置
            $configModel = $this->dao->get($map, ['*'], [], '', self::WITHOUT_SCOPES);
    ...
    public function exists(string $code, array $options = []): bool
    {
        ...
        $config = $this->dao->get($map, ['*'], [], '', self::WITHOUT_SCOPES);
        return !empty($config);
    }
    ...
    public function getItems(int $page, int $pageSize, string $groupCode, string $keyword = ''): array
    {
        ...
        foreach ($list as &$item) {
            $item['value'] = $this->formatValue($item['content']);
        }
        return [
            'items' => $list,
            'total' => $total,
     ...
     * @return mixed
     */
    private function formatValue(mixed $content): mixed
    {
        if (is_string($content) && !empty($content)) {
            $decoded = json_decode($content, true);
