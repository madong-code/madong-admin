...
class JwtToken
{
    ...
    {
        // 获取用户的所有 token 信息
        $tokens = $this->tokenStorage->getUserTokens($id, $clientType, null);
        ...
        $this->tokenStorage->deleteByUser($id, $clientType, null, null);
    ...
    public function logoutOthers(?string $id = null, ?string $clientType = null, ?string $exceptJti = null): int
    {
        ...
        // 清理用户索引
        $this->tokenStorage->deleteByUser($id, $clientType, $exceptJti, null);
    ...
    {
        return $this->tokenStorage->countByUser($id, $clientType, null);
    }
    ...
    protected function handleLoginMode(string $id, string $clientType, string $loginMode): void
    {
        switch ($loginMode) {
            ...
            case LoginMode::MULTI->value:
                ...
                if ($maxDevices > 0) {
                    $count    = $this->tokenStorage->countByUser($id, null, null);
                    if ($count >= $maxDevices) {
                        // 删除最早的设备
                        $this->tokenStorage->deleteOldest($id, null);
                    }
                }
                break;
        }
