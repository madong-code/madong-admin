...
namespace core\business\install;
...
final class InstallService
{
    ...
    /**
     * 创建 PDO 连接（替代 DataImporterService）
     */
    protected function createPdo(string $host, string $username, string $password, int $port = 3306): PDO
    {
    ...
    public function testDatabaseConnection(array $config): array
    {
        try {
            ...
            // 测试 Redis 连接（非阻塞，失败不抛异常）
            $redisHost = $config['redisHost'] ?? $config['redis_host'] ?? '';
            $redisPort = $config['redisPort'] ?? $config['redis_port'] ?? 6379;
            $redisPassword = $config['redisPassword'] ?? $config['redis_password'] ?? '';
            ...
            return [
                'connected'       => $redisConnected || true, // MySQL 成功即为整体成功
    ...
    public function install(array $params): \Generator
    {
        ...
        try {
            ...
            // 提取 Redis 配置参数
            $redisParams = $this->extractPrefixedData($params, 'redis_');
            ...
            // 前端发布兜底 (80-85%)：无论是否构建、构建是否成功，
            // 均确保 template 预置的发布产物被复制到 public/admin、public/web
            yield Sse::progress('> 发布前端静态资源', 80, [], $sessionUuid);
            yield from $this->publishFrontendAssets($sessionUuid);

            // 6. 创建 .env 文件 (85-95%)
            yield Sse::progress('> 6/6 创建.env配置文件', 90, [], $sessionUuid);
            $this->copyDatabaseTemplateAndCreateEnv($dbParams, $redisParams);

            // 所有安装步骤成功后再写入锁，避免失败安装被标记为已完成
            yield Sse::progress('> 创建安装锁', 98, [], $sessionUuid);
    ...
    private function checkFrontendProjectExists(string $projectType): bool
    {
        ...
        $projectPaths = [
            'admin'   => $baseDir . '/template/admin',
            'web'     => $baseDir . '/template/web',
            'uni-app' => $baseDir . '/template/uni-app',
        ];
        $projectDir  = $projectPaths[$projectType] ?? $baseDir . '/template/' . $projectType;
    ...
    private function installFrontendDependencies(string $projectType, string $sessionUuid): \Generator
    {
        try {
            ...
            // 使用异步执行方法提供实时输出
            $progressStep = $projectType === 'admin' ? 42 : 44;
    ...
    private function buildFrontend(string $projectType, string $sessionUuid): \Generator
    {
        try {
            ...
            // 使用异步执行方法提供实时输出
            $progressStep = $projectType === 'admin' ? 52 : 54;
     ...
     * 发布前端静态资源（兜底步骤）
     *
     * 无论是否勾选"构建前端"，均确保 template 中已存在的发布产物
     * （实时构建产物或预置 fallback 目录）被复制到 public 对应目录，
     * 避免安装成功但 public/admin、public/web 为空的情况。
     *
     * @param string $sessionUuid
     *
     * @return \Generator
     */
    private function publishFrontendAssets(string $sessionUuid): \Generator
    {
        try {
            $programs = config('terminal.frontend_programs', []);
            $progressStep = 81;

            foreach ($programs as $project => $programConfig) {
                if (!($programConfig['enabled'] ?? false)) {
                    continue;
                }

                $targetDir = $this->replacePathVariables($programConfig['target_dir'] ?? '');
                if (empty($targetDir) || is_dir($targetDir) && $this->isDirNotEmpty($targetDir)) {
                    // 目标目录已存在且有内容，说明已被构建流程填充，跳过
                    yield Sse::progress("{$project} 前端资源已就绪，跳过发布", $progressStep, [], $sessionUuid);
                    $progressStep++;
                    continue;
                }

                yield Sse::progress("发布 {$project} 前端静态资源...", $progressStep, [], $sessionUuid);
                $result = $this->migrateFrontendProgram($project);
                if ($result) {
                    yield Sse::progress("{$project} 前端静态资源发布完成", $progressStep, [], $sessionUuid);
                } else {
                    yield Sse::progress("{$project} 前端静态资源发布失败（未找到可发布的构建产物）", $progressStep, [], $sessionUuid);
                }
                $progressStep++;
            }
        } catch (Exception $e) {
            // 发布兜底失败不应中断整个安装流程
            yield Sse::progress('前端静态资源发布异常: ' . $e->getMessage(), 85, [], $sessionUuid);
        }
    }

    /**
     * 判断目录是否非空（包含除保留文件外的实际文件）
     *
     * @param string $dir
     *
     * @return bool
     */
    private function isDirNotEmpty(string $dir): bool
    {
        if (!is_dir($dir)) {
            return false;
        }
    ...
    private function migrateFrontendProgram(string $frontendType): bool
    {
        ...
        $fallbackDir = $programConfig['fallback_dir'] ?? '';
        ...
        $fallbackDir = $this->replacePathVariables($fallbackDir);
        ...
        try {
            // 优先复制实时构建产物；若构建产物目录不存在，则回退到源码包中预置的发布目录
            ...
            if (!empty($fallbackDir) && is_dir($fallbackDir)) {
                return $this->copyBuildFiles($fallbackDir, $targetDir, $copyMappings, $cleanTarget, $preserveFiles, $copyOptions);
            }
    ...
    private function replacePathVariables(string $path): string
    {
        ...
        // 前端根目录（frontend目录，兼容旧变量）
        $frontendRoot = $projectRoot . DIRECTORY_SEPARATOR . 'frontend';
        // 前端模板根目录
        $templateRoot = $projectRoot . DIRECTORY_SEPARATOR . 'template';
    ...
    {
        // 检查命令是否存在
        $command = \core\business\terminal\Terminal::getCommand($commandKey);
        if (!$command) {
    ...
    private function checkDirectoryPermissions(): array
    {
        ...
        $dirs_list = [
            ...
            [
                "path"            => dirname($root_path) . '/template/install',
                "path_name"       => "install前端目录",
                "name"            => "install_dir",
                "required"        => true,
                "permission_type" => "read_write",
                "description"     => "安装前端目录，需要读写权限",
            ],
            // 可选读权限目录
            [
    ...
    {
        // 使用 'mysql' 作为连接名，匹配 config('database.default') 返回值
        ...
        // 更新默认连接名，使静态方法也能正确找到连接
        $capsule->getContainer()['config']['database.default'] = $connectionName;
    ...
    public function saveDatabaseConfig(array $dbParams): void
    {
        ...
        $envContent = $this->replaceEnvRedisParams($envContent, [
            'host' => $dbParams['redisHost'] ?? $dbParams['redis_host'] ?? '127.0.0.1',
            'port' => $dbParams['redisPort'] ?? $dbParams['redis_port'] ?? '6379',
            'password' => $dbParams['redisPassword'] ?? $dbParams['redis_password'] ?? 'null',
        ]);
...
ENV;
    ...
    private function runAllSeeders(array $dbParams, array $adminParams, string $sessionUuid): \Generator
    {
        ...
        // 菜单 + 前台菜单 42%
        yield Sse::progress("📝 导入菜单", 42, [], $sessionUuid);
        $this->runMenu();
        
        // 字典 45%
        yield Sse::progress("📝 导入字典", 45, [], $sessionUuid);
        $this->runDict();
     ...
     * @throws \Exception
     */
    private function copyDatabaseTemplateAndCreateEnv(
        array $dbParams,
        array $redisParams = []
    ): void
    {
    ...
    private function replaceEnvDatabaseParams(string $envContent, array $dbParams): string
    {
        ...
        return $this->replaceEnvValues($envContent, $replacements);
    }
    ...
    private function replaceEnvRedisParams(string $envContent, array $redisParams): string
    {
        ...
        $password = $redisParams['password'] ?? 'null';

        $replacements = [
            'REDIS_HOST'            => $host,
            'REDIS_PORT'            => $port,
            'REDIS_PASSWORD'        => $password,
            'QUEUE_REDIS_HOST'      => "redis://{$host}:{$port}",
    ...
    /**
     * 更新或追加环境变量，兼容 KEY=value 与 KEY = value 两种格式
     */
    private function replaceEnvValues(string $envContent, array $replacements): string
    {
